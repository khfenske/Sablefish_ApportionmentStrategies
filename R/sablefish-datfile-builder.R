...
build_datfile <- function(LLsurvAC_N,LLfishAC_N) {
  ...
  #add newly generated data from OM to the .dat file components

  #====================================================================================================
  #===== Model Input Parameters/Values
  #====================================================================================================
    ######### Concatenate (first, define lines, then put together)
    testdat$endyr <- testdat$endyr + 1 
    ...
    testdat$obs_domLLsurv_biom <- unlist(testdat$obs_domLLsurv_biom, recursive = TRUE)
    testdat$obs_domLLsurv_se <- unlist( testdat$obs_domLLsurv_se)
    testdat$obs_domLLsurv_lci <- unlist(testdat$obs_domLLsurv_lci)
    testdat$obs_domLLsurv_uci <- unlist(testdat$obs_domLLsurv_uci)
    ...
    testdat$obs_LLfish_biom <- unlist(testdat$obs_LLfish_biom)
    testdat$obs_LLfish_se <- unlist( testdat$obs_LLfish_se)
    testdat$obs_LLfish_lci <- unlist(testdat$obs_LLfish_lci)
    testdat$obs_LLfish_uci <- unlist(testdat$obs_LLfish_uci)
    ...
    #testdat$oac_LLfish_bsaiwgcgeg <- unlist(testdat$oac_LLfish_bsaiwgcgeg)
    ...
    for(p in 2:length(testdat$oac_LLfish_bsaiwgcgeg[,1])){
      L_add<-paste(as.vector(testdat$oac_LLfish_bsaiwgcgeg[p,]),collapse=" ")
      L_9<-c(L_9,L_add)}
    ...
    for(q in 2:length(testdat$oac_domLLsurv_bsaiwgcgeg[,1])){
      L_add<-paste(as.vector(testdat$oac_domLLsurv_bsaiwgcgeg[q,]),collapse=" ") #ERROR HERE
      L_9<-c(L_9,L_add)}
