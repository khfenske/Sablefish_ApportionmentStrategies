...
#ABC.total <- 10000
#n.areas <- 6
#biom.data <- t(biom.dataT) #does this need to be transposed??
#biomass.cv.data <- t(biomass.cv.test.data) #does this need to be transposed?

#first - just try to get the RE model function to run...
re_apportionment <- function(ABC.total,n.areas,biom.data,biomass.cv.data) { #make up the cv stuff for now
  path<-getwd()
  #setwd(paste0(path,"/r/re/"))
  biomass.years <- as.numeric(colnames(biom.data))#(rownames(biom.data))
  biomass.n.years <- ncol(biom.data)#nrow(biom.data)
    ...
    for(i in 1:n.areas) { 
      try(
        ...
        yrs_srv<-seq(1,biomass.n.years)
              srv_est<-biom.data[,i] #was [,i]
              srv_cv<-biomass.cv.data[,i] #was [,i]
        cat(styr,"\n",endyr,"\n",nobs,"\n",yrs_srv,"\n",as.numeric(srv_est),"\n",as.numeric(srv_cv),"\n",sep=" ",file="re.dat") 
        system("re.exe")
...
re_apportionment(ABC.total[14],n.areas,t(biom.dataT[1:28,]),t(biomass.cv.test.data)) #needed to transpose the data?
    

!!!!STOP - dont run below here until RE model works!
#only want to apply RW model for 2005-2018 abcs, not the whole time series of survey biomass
    ABC.EM <- vector(length=n.areas)
    biom <- vector(length=n.areas)
    biom.prop <- vector(length=n.areas)
    biom.yrs <- length(biom.data[,1])
    for (a in 1:n.areas) {
      biom[a] <- srv_re[a,biomass.n.years-13] 
    }
    for (a in 1:n.areas) {
      biom.prop[a] <- biom[a]/sum(biom)
      ABC.EM[a]<-ABC.total*biom.prop[a]
  ...
  setwd(path)
    ...
    #run the function:
re_apportionment(ABC.total[14],n.areas,t(biom.dataT[1:28,]),t(biomass.cv.test.data)) 
