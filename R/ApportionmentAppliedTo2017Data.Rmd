...
write.csv(apptmat,"output/ApportionmentAppliedTo2017/apptmat17.csv")  #compare this one to my SQ functions
```
...
biomass.cv.test.data <- matrix(data=NA,nrow=14,ncol=n.areas.test)
biomass.cv.test.data[,1] <- rep(0.05,14)
biomass.cv.test.data[,2] <- rep(0.05,14)
biomass.cv.test.data[,3] <- rep(0.05,14)
biomass.cv.test.data[,4] <- rep(0.05,14)
biomass.cv.test.data[,5] <- rep(0.05,14)
biomass.cv.test.data[,6] <- rep(0.05,14)
...
biomassbased_apportionment <- function(ABC.total,n.areas,biom.data) { 
  srv_re<-data.frame(matrix(nrow=n.areas,ncol=max(biomass.years)-min(biomass.years)+1))  #need to identify 'biomass.years'
  srv_recv<-data.frame(matrix(nrow=n.areas,ncol=max(biomass.years)-min(biomass.years)+1))
  re_apportionment <- function(ABC.total,n.areas,biom.data,biom.cv.data) { #creating a function within a function? But where do you call this re_apport function?
    
    for(i in 1:6) { 
      try(
        {styr <-min(biomass.years)
        endyr <-max(biomass.years)
        nobs<-biomass.n.years #need to define or identify 'this'biomass.n.years' somewhere?
        yrs_srv<-seq(1,biomass.n.years)
              srv_est<-biomass.data[,i]
              srv_cv<-biomass.cv.test.data[,i]
              srv_est<-biom.data[,i]
              srv_cv<-biom.cv.data[,i]
        cat(styr,"\n",endyr,"\n",nobs,"\n",yrs_srv,"\n",as.numeric(srv_est),"\n",as.numeric(srv_cv),"\n",sep=" ",file="re.dat") #need to define 'styr', 'endyr', 'nobs', 'yrs_srv',srv_est, srv_cv
        system("re.exe")
    ...
    ### clean up ADMB mess
    flist=list.files(pattern="*.*")  #should this code (all three lines) be inside or outside the closing bracket of the function?
    flist<-flist[-grep(".exe",flist)]
    file.remove(flist)

    #run the function:
biomassbased_apportionment(ABC.total[14],n.areas,biom.dataT[11:28,],biomass.cv.test.data[1:14,]) 
...
df12 <- data.frame(matrix(data=NA, ncol=6, nrow=14)) #data frame for RE model
...
#not working yet:
for (i in 1:14) {
   allone_BS[i,] <- allone_apportionment(ABC.total[i],n.areas,1)
  }

for (i in 1:14) {
   allone_AI[i,] <- allone_apportionment(ABC.total[i],n.areas,2)
}

for (i in 1:14) {
   allone_WG[i,] <- allone_apportionment(ABC.total[i],n.areas,3)
}

for (i in 1:14) {
   allone_CG[i,] <- allone_apportionment(ABC.total[i],n.areas,4)
}

for (i in 1:14) {
   allone_WY[i,] <- allone_apportionment(ABC.total[i],n.areas,5)
}

for (i in 1:14) {
   allone_EY[i,] <- allone_apportionment(ABC.total[i],n.areas,6)
}
