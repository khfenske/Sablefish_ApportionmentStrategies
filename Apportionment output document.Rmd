...
n.apports <- 2 #number of apportionment options that have been run and are to be compiled here
...
#apply(test_ofv[forproj.styr:n.year,,],c(1,3),sum) #if value = n.sims, then all years and apportionments ran (not necessarily converged)
#apply(test_ofv[forproj.styr:n.year,,],c(2,3),sum) #if value = length(44:n.year), then all years and apportionments ran (not necessarily converged)
...
for(f in 1:n.fish){
  for(m in 1:n.area){
    for(b in 1:n.apports){
     AM_EM_abc[44:n.year, f, m,ofv.equalvec,b] <- NA 
    }
  }
}
...
``` {r, echo=FALSE}
AM_EM_abc_sum3 <- apply(AM_EM_abc[forproj.styr:n.year,,,,],c(1,3,4,5),sum,na.rm=TRUE)
diff_apport <- array(data=NA,dim=c(length(forproj.styr:n.year),n.area,n.sims,n.apports),dimnames=list(forproj.styr:n.year,1:n.area,1:n.sims,apportion.names))
for(b in 1:n.apports){
  for(i in 1:n.sims){
    for(m in 1:n.area){
      for(y in 1:length(forproj.styr:(n.year-1))){
    diff_apport[y,m,i,b] <- ((AM_EM_abc_sum3[y,m,i,b]-AM_EM_abc_sum3[y+1,m,i,b])/((AM_EM_abc_sum3[y,m,i,b]+AM_EM_abc_sum3[y+1,m,i,b])/2))*100
      }}}}

AM_EM_abc_sum3 <- apply(AM_EM_abc[forproj.styr:n.year,,,,],c(1,3,4,5),sum,na.rm=TRUE)
diff_apport2 <- array(data=NA,dim=c(length(forproj.styr:n.year),n.area,n.sims,n.apports),dimnames=list(forproj.styr:n.year,1:n.area,1:n.sims,apportion.names))
for(b in 1:n.apports){
  for(i in 1:n.sims){
    for(m in 1:n.area){
      for(y in 1:length(forproj.styr:(n.year-1))){
    diff_apport2[y,m,i,b] <- (AM_EM_abc_sum3[y,m,i,b]-AM_EM_abc_sum3[y+1,m,i,b])
      }}}}
#diff_apport
pct_chg1 <- apply(diff_apport,c(1,2,4),mean,na.rm=TRUE) #mean across sims
pct_chg2 <- apply(diff_apport[1:length(forproj.styr:(n.year-1)),,,],c(2,4),mean,na.rm=TRUE) #mean across sims and years
rownames(pct_chg2) <- area.names
pct_chg3 <- apply(diff_apport[1:length(forproj.styr:(n.year-1)),,,],4,mean,na.rm=TRUE)#mean across sims, areas, years

#using raw differences
pc2 <- apply(diff_apport2[1:length(forproj.styr:(n.year-1)),,,],c(2,4),mean,na.rm=TRUE)
pc3 <- apply(diff_apport2[1:length(forproj.styr:(n.year-1)),,,],4,mean,na.rm=TRUE)
```
The mean absolute percent change in ABC for each apportionment method, reflects the inherent variability in ABC from year to year.  
```{r, echo=FALSE}
round(pct_chg3,digits=2)
```

  
Drilling down further, the mean absolute percent change in ABC for each area and apportionment method is  
```{r, echo=FALSE}
round(pct_chg2,digits=2)
```
