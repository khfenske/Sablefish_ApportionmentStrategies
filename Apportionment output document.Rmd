...
#wd <- ("C:/Users/xkfenske/Desktop/apportionment/Sablefish_ApportionmentStrategies") #may need to manually enter your wd here
#C:\Users\xkfenske\Desktop\apportionment\Sablefish_ApportionmentStrategies
...
AM_age_LLike <- array(data=NA,dim=c(2,n.year,n.sims,length(1:n.apports),dimnames=list(c("fish","surv"),years,sims,apportion.names)))
...
AM_age_LLike[,,,b] <- readRDS(file.path(dir.temp,"age_likelihood.rds"))
...
AM_EM_fishAC[,,,,,b] <- readRDS(file.path(dir.temp,"EM_predAC.surv.rds"))
AM_EM_survAC[,,,,,b]  <- readRDS(file.path(dir.temp,"EM_predAC.fish.rds"))
...
retainedsims  #these are the proportions retained when using OFV as criteria
...
retainedsims2 #these are the proportions retained when using MGC as criteria
...
```{r, echo=FALSE}
#the spawning biomass data captures all spawning biomass estimates every time the EM is run, so each year y of the first y dimension (rows), gets longer (more y columns) as you run the sim through n.years. To extract the terminal year estimates, take [n.year,,,].
for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_spbiom[y,y,converge_id_list[[b]],b] <- NA 
    }
  }
...
for(y in 1:n.year){
  for(i in 1:n.sims){
    for(b in 1:n.apports){
      B40_ratio[y,i,b] <- AM_EM_spbiom[n.year,y,i,b]/AM_EM_B40[y,i,b] #take the EM spbiom from the terminal year of each iteration of EM over years
    }
  }
}
...
B40_ratio_sd <- vector()
B40_ratio_mean <- apply(B40_ratio[forproj.styr:n.year,,],3,mean,na.rm=TRUE)
B40_ratio_median <- apply(B40_ratio[forproj.styr:n.year,,],3,median,na.rm=TRUE) 
for(b in 1:n.apports){
B40_ratio_sd[b] <- sd(B40_ratio[forproj.styr:n.year,,b],na.rm=TRUE)
}
...
B40_ratio_mean2 <- apply(B40_ratio[forproj.styr:n.year,,],c(1,3),mean,na.rm=TRUE)
B40_ratio_median2 <- apply(B40_ratio[forproj.styr:n.year,,],c(1,3),median,na.rm=TRUE) 
...
ggplot(aes(x=factor(Year),y=SSB.B40,fill=factor(Apportionment.Method))) + 
           geom_boxplot()+
    coord_cartesian(ylim=c(0,5),xlim=10)

#now calculate the proportion of years SSB/b40 >1
#B40_props <- array(data=NA,dim=c(length(forproj.styr:n.year),n.sims,n.apports),dimnames=list(forproj.styr:n.year,sims,apporti#on.names))
...
#B40_props2 <- apply(B40_props[,,],c(1,3),sum,na.rm=TRUE)
...
#B40_props4 <- vector()
#B40_props4 <- apply(B40_props[,,],3,sum,na.rm=TRUE)
...
``` {r,echo=FALSE}
abc_allmean <- apply(apply(AM_EM_abc[44:n.year,,,,],c(1,3,4,5),sum,na.rm=TRUE),4,mean,na.rm=TRUE)
abc_allmed <- apply(apply(AM_EM_abc[44:n.year,,,,],c(1,3,4,5),sum,na.rm=TRUE),4,median,na.rm=TRUE)
...
``` {r,echo=FALSE}
AM_EM_abc_sum <- apply(AM_EM_abc[forproj.styr:n.year,,,,],c(1,4,5),sum,na.rm=TRUE)
...
temp_a <- vector() 
temp_a <- apply(app_num[,,],3,sum,na.rm=TRUE)
...
temp_b <- apply(app_num2,c(2,4),sum,na.rm=TRUE)
...
cc <- apply(bb,c(1,3,4,5),sum,na.rm=TRUE)/apply(aa,c(1,3,4,5),sum,na.rm=TRUE)
#take mean over sims
cc2 <- apply(cc,c(1,2,4),mean,na.rm=TRUE)
...
cc3 <- apply(cc2[44:n.year,,],c(2,3),mean,na.rm=TRUE)
...
for(h in 1:n.sex){
  for(a in 1:n.age){
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_pred.sel.postifq[y,a,h,converge_id_list[[b]],b] <- NA 
    }
  }
  }
}
...
for(h in 1:n.sex){
  for(a in 1:n.age){
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_pred.sel.LLsurv[y,a,h,converge_id_list[[b]],b] <- NA 
    }
  }
  }
}
...
#for(h in 1:n.sex){
 # for(a in 1:n.age){
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_q.forfish[y,converge_id_list[[b]],b] <- NA 
    }
  }
#  }
#}
melted_EMq.forfish <- melt(AM_EM_q.forfish[forproj.styr:n.year,,],na.rm=TRUE,value.name="Catchability")
...
```{r,echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_q.preifqfish[y,converge_id_list[[b]],b] <- NA 
    }
  }
melted_EMq.preifq <- melt(AM_EM_q.preifqfish[forproj.styr:n.year,,],na.rm=TRUE,value.name="Catchability")
...
```{r,echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_q.postifqfish[y,converge_id_list[[b]],b] <- NA 
    }
  }
melted_EMq.postifq <- melt(AM_EM_q.postifqfish[forproj.styr:n.year,,],na.rm=TRUE,value.name="Catchability")
...
```{r,echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_q.LLsurv[y,converge_id_list[[b]],b] <- NA 
    }
  }
melted_EMq.LLsurv <- melt(AM_EM_q.LLsurv[forproj.styr:n.year,,],na.rm=TRUE,value.name="Catchability")
...
```{r,echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_q.USJPsurv[y,converge_id_list[[b]],b] <- NA 
    }
  }
melted_EMq.USJPsurv <- melt(AM_EM_q.USJPsurv[forproj.styr:n.year,,],na.rm=TRUE,value.name="Catchability")
...
``` {r, echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_pred.srvRPN[y,y,converge_id_list[[b]],b] <- NA 
    }
  }
...
#Plot EM and OM RPN terminal year only, across sims
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_pred.fishRPW[y,y,converge_id_list[[b]],b] <- NA 
    }
  }
par(mfrow=c(3,4))

for(b in 1:n.apports){
plot(AM_EM_pred.fishRPW[n.year,15:n.year,1,b]~c(15:n.year),typ="l",ylim=c(0,2200),ylab="Fishery RPW",xlab="Year",col="red",main=apportion.names[b])
#for(y in forproj.styr:n.year){
  for(i in 1:n.sims){
...
sum_OM.predrec <- apply(OM.predrec,c(1,3,4),sum,na.rm=TRUE) #sum OM predicted recruitment across areas
#if(n.apports==1){
#par(mfrow=c(1,1))
#}
#if(n.apports==2){
...
``` {r, echo=FALSE}
    for(y in 1:n.year){
    for(b in 1:n.apports){
     AM_EM_predrec[y,y,converge_id_list[[b]],b] <- NA 
    }
  }
EM_recmed <- apply(AM_EM_predrec[n.year,,,],c(1,3),median,na.rm=TRUE)
OM_recmed <- apply(sum_OM.predrec,c(1,3),median,na.rm=TRUE)
#min(sum_OM.predrec)
#max(sum_OM.predrec)
...
``` {r, echo=FALSE}
par(mfrow=c(1,1))
EM_recmean <- apply(AM_EM_predrec[n.year,,,],c(1,3),mean,na.rm=TRUE)
OM_recmean <- apply(sum_OM.predrec,c(1,3),mean,na.rm=TRUE)
...
sum_OM_N <- apply(OM_N[,,1,,,],c(2,4,5),sum,na.rm=TRUE) #N at age 2, summed over areas
#par(mfrow=c(2,2))
#par(mfrow=c(1,1))
#i=1
#for(b in 1:n.apports){
...
### SSB time series  
``` {r, echo=FALSE}
sum_OM_ssb <- apply(OM_ssb,c(3,5,6),sum,na.rm=TRUE)
...
#OM_ssb
#AM_EM_abc
OM_ssb_area <- apply(OM_ssb,c(3,4,5,6),sum,na.rm=TRUE)
